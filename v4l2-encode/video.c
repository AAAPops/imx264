...
static int is_video_encoder(int fd, const char *name)
{
    ...
    if( found == 1)
        info("Support pixel format: '%s'", fmtdesc.description);
    else {
        dbg("There is no supported pixel format");
        return -1;
    }
    ...
    if( found == 1)
        info("Support encoded format: '%s'", fmtdesc.description);
    else {
        dbg("There is no supported encoder format");
        return -1;
    }
                        ...
                        unsigned int h)
{
    ...
    for (n = 0; n < vid->cap_buf_cnt; n++) {
        ...
        vid->cap_buf_addr[n] = mmap(NULL, buf.length,
                                    PROT_READ | PROT_WRITE, MAP_SHARED,
                                    vid->fd, buf.m.offset);

        if (vid->cap_buf_addr[n] == MAP_FAILED) {
            err("CAPTURE: MMAP failed (%s)", strerror(errno));
            return -1;
        }

        vid->cap_buf_size = buf.length;
    }
...
int video_queue_buf_cap(struct instance *i, unsigned int index)
{
    ...
    return video_queue_buf(i, index, vid->cap_buf_size,
                           vid->cap_buf_size,
